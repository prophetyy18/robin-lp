...
- the report assembly via :func:`build_robustness_report`.

Design constraints:

- **Catalogue first.** :meth:`RobustnessRunner.run` calls
  :func:`assert_catalogue_complete` before any data is touched,
  and never re-decides a scenario's outcome.
...
class RobustnessRunner:
    """The orchestrator the T064 acceptance clause calls.
