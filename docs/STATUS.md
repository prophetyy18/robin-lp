...
> 目标以 `docs/product/PROJECT_GOALS.md` 为准，任务合同以 `TODO.md` 为准。
...
- **VERIFIED**：依赖和阶段入口满足，全部验收有当前证据，TODO 可标 `[x]`。
...
- 当前分支：`main`。
- 当前已提交版本：`47f4f80`；检查时 `origin/main` 指向同一提交。
- 工作区不是 clean：存在尚未提交的目标、规格、ADR、TODO、STATUS 和少量代码/
  测试修改。因此 GitHub 上的 `main` 不是本文件描述的完整计划版本。
- 本状态更新没有授权 commit、push、merge 或部署。
...
- `.pytest_cache` 记录过 278 个 node id，并仍含两个历史失败条目；缓存不是当前
  测试通过证据。
- 当前 shell 只有 Python 3.10.12，没有项目要求的 Python 3.12，也没有 pytest、
  Ruff 或 mypy 命令。因此本次没有运行并通过完整质量门。
...
当前 TODO 中只有 T000 和 T004 保留 `[x]`。其他已有代码的任务保持 `[ ]` 并标为
`Implementation status`，因为依赖或验收尚未闭合。这不会删除已有实现；它只是
停止把实现进度误报为交付完成度。
...
- 已有 `docs/architecture.md` 和 ADR-001 至 ADR-008。
...
可以保留并优先复核，不需要因为 checkbox 撤销而重写。
...
外部证据缺失不允许把任务标 `[x]`，但也不阻止任务合同内的本地实现与 fixture
...
checksum 和残余风险。只有这些证据存在后才更新 checkbox 和本状态文件。
