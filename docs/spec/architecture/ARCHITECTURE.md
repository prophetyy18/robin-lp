...
| 6 | T109 (superseded by T112; CLI product entry awaits T113 cutover) | `robinhood_lp.orchestrator` (historical predecessor product-run lifecycle and publication path; reaches a current artifact only through the versioned read-only legacy reader, and remains Python-callable with an injected non-empty source until T113 closes that legacy CLI surface) | application / orchestration |
