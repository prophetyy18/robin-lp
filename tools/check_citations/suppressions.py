...
``owner`` and ``expiry``, plus an optional ``location`` that pinpoints the
finding to suppress. The citation checker extends the format with an
optional ``token`` field whose value is the exact token to suppress.
